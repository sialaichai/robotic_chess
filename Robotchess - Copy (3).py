...
def run_vision_game(robot, mapper):
    """
    Game Loop: ROBOT PLAYS WHITE (MOVES FIRST).
    Includes SAFETY POPUP + ROBUST CAMERA HANDLING (Fixes Crash).
    """
    ...
    # --- FIX 1: FORCE DIRECTSHOW (More stable on Windows) ---
    print("Initializing Camera (DSHOW)...")
    cap = cv2.VideoCapture(CAMERA_INDEX, cv2.CAP_DSHOW)
    
    # Camera Warmup
    time.sleep(1)
    if not cap.isOpened():
        print("CRITICAL ERROR: Camera could not open.")
        return

    board = chess.Board()
    
    # --- SAFETY TRIGGER ---
    # PREVIEW: Show the camera once so user can check alignment
    ret, preview_frame = cap.read()
    if ret:
        cv2.imshow("Chess Vision", preview_frame)
        cv2.waitKey(1) # Force the window to draw
        
    print("Waiting for user confirmation...")
    messagebox.showinfo("SAFETY CHECK", 
        "The Robot plays WHITE and moves FIRST.\n\n"
        "1. Ensure the Robot Base is on the FAR side (Black side).\n"
        "2. Ensure your hands are clear.\n\n"
        "Click OK to execute the first move.")
    ...
    print("\n--- GAME STARTED: ROBOT IS WHITE (MOVES FIRST) ---")
    
    # Initial Snapshot (Robust)
    ret, frame_base = cap.read()
    while not ret: 
        print("Waiting for camera..."); time.sleep(0.5); ret, frame_base = cap.read()
    ...
    while not board.is_game_over():
        ...
        print(f"[WHITE] Robot plays: {result.move.uci()}")
        ...
        # Robust Snapshot Update
        ret, frame_base = cap.read()
        while not ret:
            print("Warning: Camera dropped frame (Robot Turn). Retrying..."); time.sleep(0.5)
            ret, frame_base = cap.read()
        ...
        # 2. HUMAN TURN (BLACK)
        # ============================
        print("\n[BLACK] Your Turn (Move, Remove Hand, Spacebar)...")
        user_move = None
        while not user_move:
            ret, frame = cap.read()
            
            # --- FIX 2: HANDLE DROPPED FRAMES (Prevents Crash) ---
            if not ret or frame is None:
                print("Warning: Camera dropped frame (Human Turn). Retrying...")
                time.sleep(0.2)
                continue
            # -----------------------------------------------------
...
class RobotGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Button(f_grid, text="2. Set A8 (Top-L)", width=15, command=lambda: self.save_vis_pt("a8")).grid(row=0, column=0, padx=5, pady=2)
        ttk.Button(f_grid, text="3. Set H8 (Top-R)", width=15, command=lambda: self.save_vis_pt("h8")).grid(row=0, column=1, padx=5, pady=2)
        ttk.Button(f_grid, text="4. Set A1 (Bot-L)", width=15, command=lambda: self.save_vis_pt("a1")).grid(row=1, column=0, padx=5, pady=2)
        ttk.Button(f_grid, text="5. Set H1 (Bot-R)", width=15, command=lambda: self.save_vis_pt("h1")).grid(row=1, column=1, padx=5, pady=2)
        
        # Step 3: Save
        ttk.Button(f_vis, text="6. Save & Close Camera", command=self.stop_vision_stream).pack(fill='x', pady=5)

        ttk.Button(t_game, text="START CHESS GAME", command=self.start_game_thread).pack(pady=20, fill='x')
    ...
    def stop_vision_stream(self):
        if not self.vis_running: return
        
        # Check completeness
        req = ["a1", "h1", "h8", "a8"]
        missing = [r for r in req if r not in self.vis_pts]
        
        if missing:
            self.log(f"Warning: Missing {missing}. File NOT saved.")
        else:
            # Save File
            grid_data = interpolate_grid(
                self.vis_pts["a8"], self.vis_pts["h8"], 
                self.vis_pts["h1"], self.vis_pts["a1"]
            )
            with open(VISION_FILE, "w") as f: json.dump(grid_data, f)
            self.log("Vision Calibration Saved Successfully!")

        self.vis_running = False # This kills the loop and closes window

    def start_game_thread(self):
        if self.vis_running:
            self.log("Error: Please 'Save & Close' camera first.")
            return
        threading.Thread(target=run_vision_game, args=(self.robot, self.mapper)).start()
