...
class RobotMotion:
    def __init__(self, robot, mapper, stop_check_callback):
        ...
        # We assume the robot starts at Home (90,90,180,18)
    ...
    def raw_move(self, b, s, e, w, g):
        ...
        # 1. Update Internal Tracker BEFORE sending
        # This ensures that even if hardware fails, Python knows what we WANTED.
    ...
    def land_on_square(self, sq, grip_angle):
        ...
        for i in range(1, steps):
            ...
            if angles:
                # Direct send to be smooth, but ensure we pass the correct grip
                self.robot.send_cmd(angles[0], angles[1], angles[2], angles[3], grip_angle)
                time.sleep(0.05)
        
        # Final lock (Updates internal variables)
    ...
    def triple_grab(self):
        """
        MODIFIED v8.21: Gentler Grip to prevent Servo Stall/Overheat.
        """
        if self.check_stop(): return
        ...
        # 2. Gentle Crush (Only +5 degrees instead of +15)
        # Reducing this value prevents the servo from drawing max current and resetting.
    ...
    def force_open(self):
        """Spam the open command to wake up a stalled servo."""
        print("[MOTION] Forcing Grip OPEN...")
    ...
    def execute_move(self, sq_src, sq_dst):
        ...
        # SAFETY: Force open at start just in case it got stuck closed previously
        self.force_open()
        ...
        # 2. Travel & Land
        self.smart_move_xyz(x2, y2, Z_HOVER, GRIP_CLOSED_ANGLE)
        self.land_on_square(sq_dst, GRIP_CLOSED_ANGLE)
        
        # 3. Release (With Redundancy)
        self.raw_move(self.cur_b, self.cur_s, self.cur_e, self.cur_w, GRIP_OPEN_ANGLE)
        self.force_open() # Double-tap the open command
