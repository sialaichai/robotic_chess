...
class RobotMotion:
    ...
    def raw_move(self, b, s, e, w, g):
        if self.check_stop(): return
        self.cur_b, self.cur_s, self.cur_e, self.cur_w = b, s, e, w
        self.robot.gripper_state = g
        self.robot.send_cmd(b, s, e, w, g)
        self.safe_wait(1.5) # Wait for move to finish

    def smart_move_xyz(self, x, y, z, g):
        if self.check_stop(): return
        angles = self.robot.inverse_kinematics(x, y, z_height=z)
        if angles:
            self.raw_move(angles[0], angles[1], angles[2], angles[3], g)
        else:
            print(f"[WARN] Unreachable: {x:.1f}, {y:.1f}, {z:.1f}")

    # --- NEW: Forces the robot to move in a straight line ---
    def travel_straight(self, start_sq, end_sq, height, grip_angle):
        if self.check_stop(): return
        ...
        # Determine number of steps (1 step every 2 cm ensures smoothness)
        # We enforce at least 5 steps to prevent "swinging" on short moves
        steps = max(5, int(dist / 2.0))
        
        print(f"[MOTION] Pathing {start_sq}->{end_sq} in {steps} linear steps...")
        ...
        for i in range(1, steps + 1):
            ...
            # Calculate IK for this specific point on the line
            angles = self.robot.inverse_kinematics(curr_x, curr_y, z_height=height)
            if angles:
                # Send command directly (fast update, no long waits)
                self.robot.send_cmd(angles[0], angles[1], angles[2], angles[3], grip_angle)
    ...
    def land_on_square(self, sq, grip_angle):
        if self.check_stop(): return
        ...
        if sq in self.mapper.ovr and len(self.mapper.ovr[sq]) > 5:
            ...
            self.raw_move(val[3], val[4], val[5], val[6], grip_angle)
            return
        ...
        # We reuse the "travel_straight" logic for vertical descent too? 
        # Actually standard segmented descent is fine here as it's just Z changing.
        start_z = Z_HOVER
        steps = 5
        ...
        self.smart_move_xyz(tx, ty, final_z, grip_angle)

    def segmented_lift(self, x, y, grip_angle):
        if self.check_stop(): return
        target_z = Z_HOVER
        start_z = Z_GRIP 
        steps = 5
        for i in range(1, steps):
            ...
            if angles:
                self.robot.send_cmd(angles[0], angles[1], angles[2], angles[3], grip_angle)
                time.sleep(0.05)
        self.smart_move_xyz(x, y, target_z, grip_angle)

    def triple_grab(self):
        if self.check_stop(): return
        print("[MOTION] Locking Grip...")
        self.robot.send_cmd(self.cur_b, self.cur_s, self.cur_e, self.cur_w, GRIP_CLOSED_ANGLE)
        time.sleep(0.5)
        SAFE_CRUSH = min(160, GRIP_CLOSED_ANGLE + 15) 
        self.robot.send_cmd(self.cur_b, self.cur_s, self.cur_e, self.cur_w, SAFE_CRUSH)
        self.robot.gripper_state = SAFE_CRUSH
        time.sleep(0.8)

    def force_open(self):
        print("[MOTION] RECOVERY: Forcing Grip OPEN...")
        self.robot.send_cmd(self.cur_b, self.cur_s, self.cur_e, self.cur_w, GRIP_OPEN_ANGLE - 5)
        time.sleep(0.1)
        self.robot.send_cmd(self.cur_b, self.cur_s, self.cur_e, self.cur_w, GRIP_OPEN_ANGLE)
        self.robot.gripper_state = GRIP_OPEN_ANGLE
        time.sleep(0.2)
    ...
    def execute_move(self, sq_src, sq_dst):
        ...
        x1, y1, _ = self.mapper.get_coords(sq_src)
        x2, y2, _ = self.mapper.get_coords(sq_dst)

        # 1. Pick Up
        self.smart_move_xyz(x1, y1, Z_HOVER, GRIP_OPEN_ANGLE)
        self.land_on_square(sq_src, GRIP_OPEN_ANGLE)
        self.triple_grab()
        self.segmented_lift(x1, y1, GRIP_CLOSED_ANGLE)
        
        # 2. Travel (NOW LINEAR and SAFER)
        # Instead of jumping, we crawl through the air
        self.travel_straight(sq_src, sq_dst, Z_HOVER, GRIP_CLOSED_ANGLE)
        
        # 3. Land
        self.land_on_square(sq_dst, GRIP_CLOSED_ANGLE)
        
        # 4. Release
        self.raw_move(self.cur_b, self.cur_s, self.cur_e, self.cur_w, GRIP_OPEN_ANGLE)
        self.force_open()
        
        # 5. Clear Out
        self.segmented_lift(x2, y2, GRIP_OPEN_ANGLE)
    ...
    def execute_capture(self, sq_src, sq_dst):
        ...
        self.smart_move_xyz(x2, y2, Z_HOVER, GRIP_OPEN_ANGLE)
        self.land_on_square(sq_dst, GRIP_OPEN_ANGLE)
        self.triple_grab()
        self.segmented_lift(x2, y2, GRIP_CLOSED_ANGLE)
        
        # Trash Dump (Simple swing away)
        self.raw_move(180, 90, 90, 90, GRIP_CLOSED_ANGLE)
        self.force_open()
        self.raw_move(180, 90, 90, 90, GRIP_OPEN_ANGLE)
        time.sleep(0.5)
