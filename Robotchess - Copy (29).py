...
class RobotMotion:
    def __init__(self, robot, mapper, stop_check_callback):
        ...
        # --- THE MAGIC NUMBER ---
        # 145 = Too Loose (Drops)
        # 180 = Too Hard (Servo Overheats & Relaxes)
        # 155 = THE SWEET SPOT (Firm hold, no stall panic)
        self.GRIP_FORCE = 96
    ...
    # --- GRIPPER COMMANDS ---
    def lock_hand(self):
        """Closes to 155. Not 180."""
        if self.check_stop(): return
        print(f"[MOTION] Locking Hand at {self.GRIP_FORCE} (Safe Hold)...")
        ...
        time.sleep(0.5) # Short wait to avoid triggering timeout
    ...
    def segmented_lift(self, x, y):
        # Direct Lift to prevent data stutter
        self.smart_move_xyz(x, y, Z_HOVER)
    ...
    def execute_capture(self, sq_src, sq_dst):
        ...
        self.lock_hand() # Lock 155
        self.segmented_lift(x_vic, y_vic) 
...
class RobotGUI:
    ...
    def goto_sq(self):
        ...
        print(f"[GUI] Safe moving to {sq}...")
        
        # 1. Safe Vertical Lift (Manually sending current grip)
        SAFE_SHOULDER = 85 
        ...
        # 2. Travel
        # We manually set the motion's grip to match the GUI's current state
        motion.current_grip = self.g 
        motion.travel_straight(sq, sq) 
        
        # 3. Land
        motion.land_on_square(sq)
        
        angles = self.robot.inverse_kinematics(x, y, z_height=z_surface)
