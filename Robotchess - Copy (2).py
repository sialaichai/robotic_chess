...
def run_vision_game(robot, mapper):
    """Main Game Loop with Vision (Updated with Safe Gripper Logic)."""
    ...
    # --- GRIPPER TUNING (MATCHING YOUR SIMULATOR) ---
    GRIP_WIDE   = 30    # Fully Open (Capture & Approach)
    GRIP_SAFE   = 30    # Fully Open (Release)
    GRIP_CLOSED = 105   # Tight Grip

    # --- TRACKING VARIABLES ---
    # We need to remember where the arm is, just like in _sim
    cur_b, cur_s, cur_e, cur_w = 90, 90, 180, 18 # Start at Home
    ...
    # --- HELPER: Dive to Square ---
    def land(sq, grip_angle):
        nonlocal cur_b, cur_s, cur_e, cur_w
        # 1. Try Angle Override (Pinned)
        if sq in mapper.ovr:
            val = mapper.ovr[sq]
            if len(val) > 7 and val[7] == "ANGLES":
                b, s, e, w = val[3:7]
                raw_move(b, s, e, w, grip_angle) 
                return
        ...
        x, y = r[0], r[1]
        z = Z_GRIP
        if len(r) > 2 and r[2]: z = r[2] 
        else:
            dist = math.sqrt(x**2 + y**2)
            z = Z_GRIP + max(MAX_TILT_DROP, (dist**2 * TILT_CURVE) + (dist * TILT_SLOPE) + TILT_INTERCEPT)
        
        angles = robot.inverse_kinematics(x, y, z_height=z)
        if angles:
            b, s, e, w = angles
            raw_move(b, s, e, w, grip_angle)
        else:
            print(f"GAME ERROR: Could not reach {sq}")
    ...
    while not board.is_game_over():
        ...
        if is_capture:
            ...
            cur = robot.inverse_kinematics(x2, y2, z_height=Z_TRAVEL)
            if cur: cur_b, cur_s, cur_e, cur_w = cur

            # Dive WIDE
            land(sq_dst, GRIP_WIDE) 
            ...
            # Trash Bin
            raw_move(180, 90, 90, 90, GRIP_CLOSED) 
            time.sleep(0.5)
            raw_move(180, 90, 90, 90, GRIP_WIDE) 
            time.sleep(0.5)
        ...
        # Approach Source: Ensure WIDE
        raw_move(cur_b, cur_s, cur_e, cur_w, GRIP_WIDE)
        robot.move_to_xyz(x1, y1, z=Z_TRAVEL); time.sleep(1)
        cur = robot.inverse_kinematics(x1, y1, z_height=Z_TRAVEL)
        if cur: cur_b, cur_s, cur_e, cur_w = cur
        ...
        # Release (GRIPPER FIRST - THE FIX)
        # Use stored angles if available (Pinned) or current angles
        target_b, target_s, target_e, target_w = cur_b, cur_s, cur_e, cur_w
        if sq_dst in mapper.ovr and len(mapper.ovr[sq_dst]) > 7:
             val = mapper.ovr[sq_dst]
             target_b, target_s, target_e, target_w = val[3:7] 

        # Open and Wait
        robot.send_cmd(target_b, target_s, target_e, target_w, GRIP_SAFE)
        robot.gripper_state = GRIP_SAFE
        time.sleep(1.0) 
        
        # Lift and Home
        robot.move_to_xyz(x2, y2, z=Z_TRAVEL); time.sleep(0.5)
        robot.home()
        cur_b, cur_s, cur_e, cur_w = 90, 90, 180, 18 # Reset tracker to home
        
        board.push(result.move)
        time.sleep(1.5); ret, frame_base = cap.read() # Update frame
...
class RobotGUI:
    ...
    def goto_sq(self):
        ...
        # 1. Check if we have "Angle Overrides"
        # We peek directly into the overrides dictionary
        is_angle_mode = False
        saved_angles = []
        ...
        if sq in self.mapper.ovr:
            ...
            if len(val) > 5 and val[7] == "ANGLES":
                is_angle_mode = True
                saved_angles = val[3:7] # Extract [b, s, e, w]

        # 2. Get Coordinates (Standard Logic)
        ...
        # A. Lift to Travel Height (Safety Move)
        self.robot.move_to_xyz(x, y, z=Z_TRAVEL)
        time.sleep(0.5)
        
        if is_angle_mode:
            # B. THE FIX: Dive using the EXACT saved angles
            # No Inverse Kinematics. No Math. Just Replay.
            b, s, e, w = saved_angles
            self.robot.send_cmd(b, s, e, w, 180)
            
            # Sync GUI variables so Jogging is safe
            self.b, self.s, self.e, self.w = b, s, e, w
            self.g = 180
            self.log(f"Arrived {sq} using EXACT SAVED ANGLES.")
            
        else:
            # Standard Math Mode (for unpinned squares)
  ...
  #      json.dump(d, open(OVERRIDES_FILE, 'w')); self.mapper = BoardMapper(); self.log(f"Pinned {sq} at Z={z:.1f}")
    ...
    def pin_sq(self):
        ...
        # 1. Get Coordinates (Keep this for the Chess Engine logic)
        x, y, z = get_xy_from_angles(self.b, self.s, self.e, self.w)
        
        # 2. Get Angles (This is the NEW "Magic" part)
        # We save a special list with 8 items: [x,y,z, b,s,e,w, "ANGLES"]
        ...
        self.mapper = BoardMapper() # Reload mapper
    ...
    def _sim(self):
        ...
        # DEBUG: Check Capture Flag
        raw_cap = self.chk_cap.get()
        cap = (raw_cap == 1) 
        print(f"DEBUG: Move={m}, Capture={cap}")

        # --- TUNING: GRIPPER ANGLES ---
        # 30 = Fully Open (Matches Pulse 400)
        # 105 = Closed (Matches Pulse 1400)
        
        GRIP_WIDE   = 30    # Capture Open
        GRIP_SAFE   = 30    # <--- CHANGED: Set to Fully Open to guarantee release
        GRIP_CLOSED = 105   # Tight Grip

        # --- HELPER: Execute Move & Update ALL States ---
        def raw_move(b, s, e, w, g):
            # 1. Send Command
            ...
            self.b, self.s, self.e, self.w = b, s, e, w
            self.g = g
            # 3. Update Robot Memory (Crucial for move_to_xyz)
            self.robot.gripper_state = g 
            # 4. Wait for Servo to finish BEFORE doing anything else
            time.sleep(0.8) 
        ...
        def land(sq, grip_angle):
            ...
            if angles:
                ...
                print(f"SIM ERROR: Could not reach {sq}")
        ...
        if cap:
            ...
            cur = self.robot.inverse_kinematics(x2, y2, z_height=Z_TRAVEL)
            if cur: self.b, self.s, self.e, self.w = cur
        ...
        # 1. Approach Source: Ensure WIDE
        raw_move(self.b, self.s, self.e, self.w, GRIP_WIDE)
        self.robot.move_to_xyz(x1, y1, z=Z_TRAVEL); time.sleep(1)
        
        cur = self.robot.inverse_kinematics(x1, y1, z_height=Z_TRAVEL)
        if cur: self.b, self.s, self.e, self.w = cur
        ...
        land(m[2:4], GRIP_CLOSED) # Dive Holding
        
        # 6. Release (GRIPPER FIRST)
        # We now use the Wide Angle (30) to guarantee it drops.
        print(f"Releasing at {m[2:4]} with Angle {GRIP_SAFE}...")
        
        target_b, target_s, target_e, target_w = self.b, self.s, self.e, self.w
        if m[2:4] in self.mapper.ovr and len(self.mapper.ovr[m[2:4]]) > 7:
             val = self.mapper.ovr[m[2:4]]
             target_b, target_s, target_e, target_w = val[3:7] 

        # A. OPEN GRIPPER
        self.robot.send_cmd(target_b, target_s, target_e, target_w, GRIP_SAFE)
        self.robot.gripper_state = GRIP_SAFE
        self.g = GRIP_SAFE
        
        # B. WAIT FOR OPEN (Crucial Step)
        time.sleep(1.0) 
        
        # 7. Lift and Home
        # Arm will only start moving AFTER the sleep above is finished.
        self.robot.move_to_xyz(x2, y2, z=Z_TRAVEL); time.sleep(0.5)
        self.robot.home()
        self.b, self.s, self.e, self.w = 0, 90, 90, 90
